...
publish_source = 'inform.kz'
country = 'Kazakhstan'
language = 'English'
images_path = []

rss_page = c.download_page('https://inform.kz/rss/eng.xml')
...
for i in rss_page.split('<item>')[1:]:
    ...
    source_headline = c.strip_html(source_headline)
    ...
    try:
        date_time_str = c.scrap('<pubDate>(.*?)\+', i) #Wed, 12 May 2021 11:23:10
    ...
    source_content = c.scrap('class="frame_news_article">.*?<p>(.*?)<script\s*type="',page)
    if not source_content:
        source_content = c.scrap('class="article_news_body">(.*?)<script\s*type="',page)
