...
publish_source = 'ukrinform.net'
country = 'Ukraine'
language = 'English'
images_path = []

home_page = c.download_page('https://www.ukrinform.net/block-lastnews')
home_page= c.scrap('class="otherNews">(.*?)<div\s*data\-name="inscroll_content">',home_page)

for i in home_page.split('<article data-id=')[0:]:
    
    source_link = c.scrap('<a\s*href="(.*?)"', i)
    if 'http' not in source_link:
        source_link = 'https://www.ukrinform.net' + source_link 
    ...
    try:
        date_time_str = c.scrap('datetime="(.*?)\+',page) #20210515T145400
...
#         break    
    ...
    source_content = c.scrap('\s*<div\s*class="newsText">(.*?)\s*<aside\s*class="share">',page)
